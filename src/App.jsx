...
function writeLocalStorage(key, value) {
  try { window.localStorage.setItem(key, typeof value === "string" ? value : JSON.stringify(value)); return true; } catch (e) { return false; }
}
  ...
  async function onBackupFileChange(e) {
    ...
    try {
      ...
      if (!data || !Array.isArray(data.projects)) {
        throw new Error("Invalid backup: projects data is missing");
      }

      setProjects(data.projects);
      markDirty();
      showToast("Backup restored — click Save changes");
    } catch (error) {
      console.error("Backup restore error:", error);
      showToast("That file doesn't look like a valid Galpotori backup", "error");
    } finally {
      setBusy(false);
    }
  }
  ...
  return (
    <div className="space-y-5 fade-up fade-up-1">
      <div className="bg-white rounded-2xl p-5 soft-shadow">
        ...
        <p className="text-xs text-[#A79E89] mb-4">
          Save all your clients and projects to a JSON file. Restore it later if you switch phones or reinstall the app.
        </p>
        ...
        <div className="flex gap-2.5">
          <button
            ...
            disabled={busy}
            className="flex-1 flex items-center justify-center gap-1.5 text-sm font-medium py-3 rounded-xl press disabled:opacity-50"
            style={{ background: "linear-gradient(135deg, #DCA84E, #B8842E)", color: "#FBF8F2" }}
          >
          ...
          <button
            ...
            disabled={busy}
            className="flex-1 flex items-center justify-center gap-1.5 text-sm font-medium py-3 rounded-xl press disabled:opacity-50"
            style={{ background: "#FBF8F2", color: "#262420", boxShadow: "inset 0 0 0 1px #F1EBDA" }}
          >
        ...
        <div className="text-[10.5px] text-[#C9BE9E] mt-3">
          Restore replaces the currently loaded projects. After restoring, press Save changes to keep them permanently.
        </div>
      </div>
...
function submit() {
  ...
  if (paid !== "" && (Number(paid) < 0 || Number(paid) > Number(price))) {
    showToast("Amount paid cannot be greater than total price", "error");
    return;
  }
